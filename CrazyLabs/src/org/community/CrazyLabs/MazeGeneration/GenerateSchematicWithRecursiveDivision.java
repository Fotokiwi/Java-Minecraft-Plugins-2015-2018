...
import org.community.CrazyLabs.*;

public class GenerateSchematicWithRecursiveDivision{

	CrazyLabs plugin = null;

	static final String MAZE_WALL = "STONE";
	static final String MAZE_PATH = "AIR";
	int rows;
	int cols;
	int act_rows;
	int act_cols;
	...
	public GenerateSchematicWithRecursiveDivision(CrazyLabs plugin, int row, int col) {
		this.plugin = plugin;
	...
	public String[][] getMaze() {
		String[][] structure = new String[this.rows][this.cols];
		for(int i=0; i<rows; i++){
			for(int j=0; j<cols; j++){
				if(board[i][j].equalsIgnoreCase("AIR")) { /* AIR erzeugt eine Liste mit Gängen, STONE eine Liste mit Wänden! */
					//structure.add(i + "~" + j + "~0"); /* Ermöglicht die Angabe der Ebene */
					structure[i][j] = "Module"; /* Eine rein zweidimensionale Darstellung */
				}
			}
		}
		return structure;
