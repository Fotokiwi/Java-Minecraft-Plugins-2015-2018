...
public class InventoryClickResult
{
	...
	public InventoryClickResult(InventoryClickEvent event)
	{
	...
	private void calcChangeInChest(InventoryClickEvent event)
	{
		...
		if(event.isShiftClick()) //Shiftklick
		{
			if(event.getRawSlot()>event.getInventory().getSize()) //im Spieler Inventar
			{
				...
				free = freeRoomForItem(event.getInventory(),event.getCurrentItem()); //Berechnen wie viel übertragen wird
				if (free == 0)
				{
					this.itemsPut=null;
					this.itemsTook=null;
