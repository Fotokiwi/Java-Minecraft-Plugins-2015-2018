...
public class ItemTypeParser {
	...
	public ItemTypeParser(fourWays plugin)
	{
		this.plugin = plugin;
	}
	
	public String blockLibrary = "WOOD,LOG,LEAVES,SAPLING,SANDSTONE,LONG_GRASS,WOOL,CROPS,DOUBLE_STEP,STEP,SMOOTH_BRICK,WOOD_DOUBLE_STEP,WOOD_STEP,INK_SACK";
	
	@SuppressWarnings("deprecation")
	public String getBlockType(Block block, ItemStack item) {
	...
	public void setBlockData(Block block, String type, String data) {
		block.setType(Material.getMaterial(type));
		if(block.getType() == Material.LOG || block.getType() == Material.WOOD || block.getType() == Material.SAPLING || block.getType() == Material.LEAVES) {			
			BlockState bs = block.getState();
			bs.setData((MaterialData)new Tree(getTreeMaterial(data)));
			bs.update(true);
			return;
		}
	...
	private TreeSpecies getTreeMaterial(String material) {
			
			if(material.equalsIgnoreCase("")) {
				return TreeSpecies.GENERIC;
			}
			if(material.equalsIgnoreCase("BIRCH")) {
				return TreeSpecies.BIRCH;
			}
			if(material.equalsIgnoreCase("JUNGLE")) {
				return TreeSpecies.JUNGLE;
			}
			if(material.equalsIgnoreCase("REDWOOD")) {
	...
	@SuppressWarnings("deprecation")
	public int getTreeSubId(Block block) {
		Tree tree = null;
		
		if(block.getType().toString().equalsIgnoreCase("LEAVES_2")) {
			byte subID = block.getData();
			if(subID == 0 || subID == 4 || subID == 8 || subID == 12)
				return 4;
			if(subID == 1 || subID == 5 || subID == 9 || subID == 13)
				return 5;
			return 0;
		}
