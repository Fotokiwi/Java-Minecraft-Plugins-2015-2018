...
public class BlazeChanger {

	@SuppressWarnings("unused")
	private MonsterSpawner plugin;

	public BlazeChanger(MonsterSpawner plugin){
		this.plugin=plugin;
	}

	private static final UUID movementSpeedUID = UUID.fromString("206a89dc-ae78-4c4d-b42c-3b31db3f5a7c");
	private static final UUID maxHealthUID = UUID.fromString("206abbb23c-aD78-41cd-b42c-3b31b33f5a7c");
	...
	private static final UUID attackDamageUID = UUID.fromString("206a8666-abb5-4c4d-b42c-3b31db3f5a7c");

	public void changeBlaze(Blaze blaze, Spawner spawner){
		...
		try {
			...
			if(!spawner.getPotion().equalsIgnoreCase("none")){
				blaze.addPotionEffect(new PotionEffect(PotionEffectType.getByName(spawner.getPotion()), 8640000, 1));
			}

			double skelx = blaze.getLocation().getX();
			double skely = blaze.getLocation().getY();
			double skelz = blaze.getLocation().getZ();
			Boolean found = false;
			for (int i = 0; i < 100; i++){
				List<Entity> entities = blaze.getNearbyEntities(skelx+i, skely+i, skelz+i);
				for (Entity e : entities) {
					if (e.getType().equals(EntityType.PLAYER)) {
						blaze.setTarget((Player) e);
						found = true;
						break;
					}
				}
			...
			e.printStackTrace();
			Bukkit.broadcastMessage("Fehler im Change Blaze");
		}
