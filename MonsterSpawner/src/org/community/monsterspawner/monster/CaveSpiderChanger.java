...
public class CaveSpiderChanger {

	@SuppressWarnings("unused")
	private MonsterSpawner plugin;


	public CaveSpiderChanger(MonsterSpawner plugin) {
		this.plugin = plugin;
	}

	private static final UUID movementSpeedUID = UUID.fromString("206a89dc-ae78-4c4d-b42c-3b31db3f5a7c");
	private static final UUID maxHealthUID = UUID.fromString("20666623c-aD78-41cd-b42c-3b31a33f5a7c");
	...
	private static final UUID attackDamageUID = UUID.fromString("206a8666-a321-4c4d-b42c-3b31db3f5a7c");	

	public void changeCaveSpider(CaveSpider cavespider, Spawner spawner){

		EntityEquipment equipment = cavespider.getEquipment();
		...
		try {
			...
			if(spawner.getSpecial()){
				Location moblocation = cavespider.getLocation();
				Skeleton skeleton = (Skeleton)Bukkit.getServer().getWorld(spawner.getWorldName()).spawnEntity(moblocation, EntityType.SKELETON);
				EntityEquipment equipmentsit = skeleton.getEquipment();
				equipmentsit.setItemInHand(new ItemStack(Material.BOW));
				cavespider.setPassenger(skeleton);
			}
			...
			//Attribut Änderungen
			Entity entity = (Entity) cavespider;
			...
			if(!spawner.getPotion().equalsIgnoreCase("none")){
				cavespider.addPotionEffect(new PotionEffect(PotionEffectType.getByName(spawner.getPotion()), 8640000, 1));
			}


			double skelx = cavespider.getLocation().getX();
			double skely = cavespider.getLocation().getY();
			double skelz = cavespider.getLocation().getZ();
			Boolean found = false;

			for (int i = 0; i < 100; i++){
				List<Entity> entities = cavespider.getNearbyEntities(skelx+i, skely+i, skelz+i);
				for (Entity e : entities) {
					if (e.getType().equals(EntityType.PLAYER)) {
						cavespider.setTarget((Player) e);
						found = true;
						break;
					}
				}
			...
			e.printStackTrace();
			Bukkit.broadcastMessage("Fehler im Change Spider");
		}
	}
}