...
public class EnderDragonChanger {

	@SuppressWarnings("unused")
	private MonsterSpawner plugin;

	public EnderDragonChanger(MonsterSpawner plugin) {
		this.plugin = plugin;
	}

	private static final UUID movementSpeedUID = UUID.fromString("206a89dc-ae78-4c4d-b42c-3b31db3f5a7c");
	private static final UUID maxHealthUID = UUID.fromString("206dddd23c-aD78-41cd-b42c-3b31d33f5a7c");
	...
	private static final UUID attackDamageUID = UUID.fromString("206a8666-add5-4c4d-b42c-3b31db3f5a7c");

	public void changeEnderDragon(EnderDragon dragon, Spawner spawner){
		EntityEquipment equipment = dragon.getEquipment();
		try {
			...
			if(spawner.getSpecial()){
				Location moblocation = dragon.getLocation();
			...
			if(!spawner.getPotion().equalsIgnoreCase("none")){
				dragon.addPotionEffect(new PotionEffect(PotionEffectType.getByName(spawner.getPotion()), 8640000, 1));
			}

		}
		catch (Throwable e) {
			e.printStackTrace();
			Bukkit.broadcastMessage("Fehler im Change EnderDragon");
		}
	}
}