...
public class GhastChanger {

	@SuppressWarnings("unused")
	private MonsterSpawner plugin;

	public GhastChanger(MonsterSpawner plugin) {
		this.plugin = plugin;
	}

	private static final UUID movementSpeedUID = UUID.fromString("206a89dc-ae78-4c4d-b42c-3b31db3f5a7c");
	private static final UUID maxHealthUID = UUID.fromString("206652123c-aD78-41cd-b42c-3b31a33f5a7c");
	...
	private static final UUID attackDamageUID = UUID.fromString("206a8666-bcc5-4c4d-b42c-3b31db3f5a7c");

	public void changeGhast(Ghast ghast, Spawner spawner){
		...
		try {
			...
			if(!spawner.getPotion().equalsIgnoreCase("none")){
				ghast.addPotionEffect(new PotionEffect(PotionEffectType.getByName(spawner.getPotion()), 8640000, 1));
			}

		}
		catch (Throwable e) {
			e.printStackTrace();
			Bukkit.broadcastMessage("Fehler im Change Ghast");
		}
	}
}