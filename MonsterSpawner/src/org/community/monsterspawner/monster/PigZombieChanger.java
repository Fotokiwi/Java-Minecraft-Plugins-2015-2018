...
public class PigZombieChanger {

	@SuppressWarnings("unused")
	private MonsterSpawner plugin;


	public PigZombieChanger(MonsterSpawner plugin) {
		this.plugin = plugin;
	}

	private static final UUID movementSpeedUID = UUID.fromString("206a89dc-ae78-4c4d-b42c-3b31db3f5a7c");
	private static final UUID maxHealthUID = UUID.fromString("206aA9d23c-aD78-41cd-b42c-3b31833f5a7c");
	...
	private static final UUID attackDamageUID = UUID.fromString("206a8666-a775-4c4d-b42c-3b31db3f5a7c");

	public void changePigZombie(PigZombie pigzombie, Spawner spawner){
		...
		try {
			...
			//Attribut Änderungen
			Entity entity = (Entity) pigzombie;
			...
			if(!spawner.getPotion().equalsIgnoreCase("none")){
				pigzombie.addPotionEffect(new PotionEffect(PotionEffectType.getByName(spawner.getPotion()), 8640000, 1));
			}

			double skelx = pigzombie.getLocation().getX();
			double skely = pigzombie.getLocation().getY();
			double skelz = pigzombie.getLocation().getZ();
			pigzombie.setAngry(true);

			Boolean found = false;
			for (int i = 0; i < 100; i++){
				List<Entity> entities = pigzombie.getNearbyEntities(skelx+i, skely+i, skelz+i);
				for (Entity e : entities) {
					if (e.getType().equals(EntityType.PLAYER)) {
						pigzombie.setTarget((Player) e);
						found = true;
						break;
					}
				}
			...
			e.printStackTrace();
			Bukkit.broadcastMessage("Fehler im Change PigZombie");
		}
	}
}