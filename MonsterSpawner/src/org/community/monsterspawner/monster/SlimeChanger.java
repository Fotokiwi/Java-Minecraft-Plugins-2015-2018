...
public class SlimeChanger {

	@SuppressWarnings("unused")
	private MonsterSpawner plugin;


	public SlimeChanger(MonsterSpawner plugin) {
		this.plugin = plugin;
	}


	private static final UUID movementSpeedUID = UUID.fromString("206a89dc-ae78-4c4d-b42c-3b31db3f5a7c");
	private static final UUID maxHealthUID = UUID.fromString("206a2d23c-aD78-41cd-b42c-3b31133f5a7c");
	...
	private static final UUID attackDamageUID = UUID.fromString("206a8666-ab75-4c4d-b42c-3b31db3f5c7c");

	public void changeSlime(Slime slime, Spawner spawner){
		...
		try {
			...
			if(spawner.getslimeSize()>0){
			 slime.setSize(spawner.getslimeSize());  	
			}

			//Attribut Änderungen
			Entity entity = (Entity) slime;
			...
			if(!spawner.getPotion().equalsIgnoreCase("none")){
				slime.addPotionEffect(new PotionEffect(PotionEffectType.getByName(spawner.getPotion()), 8640000, 1));
			}

		}
		catch (Throwable e) {
			e.printStackTrace();
			Bukkit.broadcastMessage("Fehler im Change Slime");
		}
	}
}