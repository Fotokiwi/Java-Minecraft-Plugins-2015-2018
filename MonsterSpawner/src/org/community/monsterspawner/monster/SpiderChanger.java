...
public class SpiderChanger {

	@SuppressWarnings("unused")
	private MonsterSpawner plugin;


	public SpiderChanger(MonsterSpawner plugin) {
		this.plugin = plugin;
	}

	private static final UUID movementSpeedUID = UUID.fromString("206a89dc-ae78-4c4d-b42c-3b31db3f5a7c");
	private static final UUID maxHealthUID = UUID.fromString("206abbd23c-aD78-41cd-b42c-3b39933f5a7c");
	...
	private static final UUID attackDamageUID = UUID.fromString("206a8666-cc75-4c4d-b42c-3b31db3f5a7c");	

	public void changeSpider(Spider spider, Spawner spawner){
		...
		try {
			...
			if(spawner.getSpecial()){
				Location moblocation = spider.getLocation();
			...
			if(!spawner.getPotion().equalsIgnoreCase("none")){
				spider.addPotionEffect(new PotionEffect(PotionEffectType.getByName(spawner.getPotion()), 8640000, 1));
			}


			double skelx = spider.getLocation().getX();
			double skely = spider.getLocation().getY();
			double skelz = spider.getLocation().getZ();
			Boolean found = false;

			for (int i = 0; i < 100; i++){
				List<Entity> entities = spider.getNearbyEntities(skelx+i, skely+i, skelz+i);
				for (Entity e : entities) {
					if (e.getType().equals(EntityType.PLAYER)) {
						spider.setTarget((Player) e);
						found = true;
						break;
					}
				}
