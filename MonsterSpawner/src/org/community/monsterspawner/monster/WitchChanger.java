...
public class WitchChanger {

	@SuppressWarnings("unused")
	private MonsterSpawner plugin;


	public WitchChanger(MonsterSpawner plugin) {
		this.plugin = plugin;
	}

	private static final UUID movementSpeedUID = UUID.fromString("206a89dc-ae78-4c4d-b42c-3b31db3f5a7c");
	private static final UUID maxHealthUID = UUID.fromString("206a03d23c-aD78-41cd-b42c-3b31a33f5a7c");
	...
	public void changeWitch(Witch witch, Spawner spawner){
		...
		try {
			...
			if(!spawner.getPotion().equalsIgnoreCase("none")){
				witch.addPotionEffect(new PotionEffect(PotionEffectType.getByName(spawner.getPotion()), 8640000, 1));
			}


			double skelx = witch.getLocation().getX();
			double skely = witch.getLocation().getY();
			double skelz = witch.getLocation().getZ();
			Boolean found = false;

			for (int i = 0; i < 100; i++){
				List<Entity> entities = witch.getNearbyEntities(skelx+i, skely+i, skelz+i);
				for (Entity e : entities) {
					if (e.getType().equals(EntityType.PLAYER)) {
						witch.setTarget((Player) e);
						found = true;
						break;
					}
				}
			...
			e.printStackTrace();
			Bukkit.broadcastMessage("Fehler im Change Witch");
		}
	}
}