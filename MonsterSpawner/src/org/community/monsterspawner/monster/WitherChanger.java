...
public class WitherChanger {

	@SuppressWarnings("unused")
	private MonsterSpawner plugin;

	public WitherChanger(MonsterSpawner plugin) {
		this.plugin = plugin;
	}

	private static final UUID movementSpeedUID = UUID.fromString("206a89dc-ae78-4c4d-b42c-3b31db3f5a7c");
	private static final UUID maxHealthUID = UUID.fromString("206a11d23c-aD78-41cd-b42c-3b31c43f5a7c");
	...
	private static final UUID attackDamageUID = UUID.fromString("206a8666-a9c5-4c4d-b42c-3b31db3f5a7c");

	public void changeWither(Wither wither, Spawner spawner){
		...
		try {
			...
			if(!spawner.getPotion().equalsIgnoreCase("none")){
				wither.addPotionEffect(new PotionEffect(PotionEffectType.getByName(spawner.getPotion()), 8640000, 1));
			}

			double skelx = wither.getLocation().getX();
			double skely = wither.getLocation().getY();
			double skelz = wither.getLocation().getZ();
			Boolean found = false;
			for (int i = 0; i < 100; i++){
				List<Entity> entities = wither.getNearbyEntities(skelx+i, skely+i, skelz+i);
				for (Entity e : entities) {
					if (e.getType().equals(EntityType.PLAYER)) {
						wither.setTarget((Player) e);
						found = true;
						break;
					}
				}
			...
			e.printStackTrace();
			Bukkit.broadcastMessage("Fehler im Change Wither");
		}
	}
}