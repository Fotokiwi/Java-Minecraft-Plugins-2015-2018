...
public class WolfChanger {

	@SuppressWarnings("unused")
	private MonsterSpawner plugin;


	public WolfChanger(MonsterSpawner plugin) {
		this.plugin = plugin;
	}

	private static final UUID movementSpeedUID = UUID.fromString("206a89dc-ae78-4c4d-b42c-3b31db3f5a7c");
	private static final UUID maxHealthUID = UUID.fromString("206cccd23c-aD78-41cd-b42c-3b31633f5a7c");
	...
	private static final UUID attackDamageUID = UUID.fromString("206a8666-9975-4c4d-b42c-3b31db3f5a7c");

	public void changeWolf(Wolf wolf, Spawner spawner){
		...
		try {
			...
			if(!spawner.getPotion().equalsIgnoreCase("none")){
				wolf.addPotionEffect(new PotionEffect(PotionEffectType.getByName(spawner.getPotion()), 8640000, 1));
			}

			double skelx = wolf.getLocation().getX();
			double skely = wolf.getLocation().getY();
			double skelz = wolf.getLocation().getZ();
			Boolean found = false;
			wolf.setAngry(true);

			for (int i = 0; i < 100; i++){
				List<Entity> entities = wolf.getNearbyEntities(skelx+i, skely+i, skelz+i);
				for (Entity e : entities) {
					if (e.getType().equals(EntityType.PLAYER)) {
						wolf.setTarget((Player) e);
						wolf.damage(0D,(Player) e);
						found = true;
						break;
					}
				}
			...
			e.printStackTrace();
			Bukkit.broadcastMessage("Fehler im Change Wolf");
		}
	}
}