...
public class ZombieChanger {

	@SuppressWarnings("unused")
	private MonsterSpawner plugin;


	public ZombieChanger(MonsterSpawner plugin) {
		this.plugin = plugin;
	}

	private static final UUID movementSpeedUID = UUID.fromString("206a89dc-ae78-4c4d-b42c-3b31db3f5a7c");
	private static final UUID maxHealthUID = UUID.fromString("206affd23c-aD78-41cd-b42c-3b3133f5a7c");
	...
	private static final UUID attackDamageUID = UUID.fromString("206a8666-a245-4c4d-b42c-3b31db3f5a7c");

	public void changeZombie(Zombie zombie, Spawner spawner){
		...
		try {
			...
			if(!spawner.getPotion().equalsIgnoreCase("none")){
				zombie.addPotionEffect(new PotionEffect(PotionEffectType.getByName(spawner.getPotion()), 8640000, 1));
			}
			double skelx = zombie.getLocation().getX();
			double skely = zombie.getLocation().getY();
			double skelz = zombie.getLocation().getZ();
			Boolean found = false;
			for (int i = 0; i < 100; i++){
				List<Entity> entities = zombie.getNearbyEntities(skelx+i, skely+i, skelz+i);
				for (Entity e : entities) {
					if (e.getType().equals(EntityType.PLAYER)) {
						zombie.setTarget((Player) e);
						found = true;
						break;
					}
				}
			...
			e.printStackTrace();
			Bukkit.broadcastMessage("Fehler im Change Zombie");
		}
	}
}